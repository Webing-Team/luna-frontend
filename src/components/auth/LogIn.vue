...
<template> 
    <header class="main-header"> 
        <router-link to="/"> 
            <img class="mt-[44px] ml-[46px]" src="/src/assets/img/maki_arrow.png" alt=""> 
        </router-link> 
        <div class="flex justify-center mt-24 gap-[350px]"> 
            <img src="/src/assets/img/moon.png" class="mt-[70px]" alt=""> 
            <div 
                class="k h-[453px] w-[413px] bg-[#181818] rounded-[55px] mt-[20px] shadow-[1px_1px_5px_2px_rgba(0,0,0,0.5)]"> 
                ...
                <form @submit.prevent="login" class="flex flex-col items-center mt-[26px]"> 
                    ...
                    <button type="submit" 
                        class="flex justify-center text-white pt-[10px] w-[346px] h-[45px] rounded-[10px] bg-[#23415B]  mt-[5px]">Log 
                        In</button> 
                </form> 
            </div> 
        </div> 
        <div class="text-container flex justify-center text-center mt-1 ml-[730px]"> 
            <p class="main-header__text__forgot-password font-inter text-base text-[#FFFFFF] gap-5"> 
                You don’t have an account? <router-link class="main-header__link text-[rgb(115,155,185)]" 
                    to="/register">Sign Up!</router-link> 
            </p> 
        </div> 
        <p 
            class="main-header__text__forgot-password text-container flex justify-center text-center mt-[4px] ml-[730px] font-inter text-base text-[#8C8C8C] gap-5"> 
            Forgot password? 
        </p> 
    </header> 
</template> 
...
const login = async () => { 
    ...
    try { 
        ...
        if (response.status === "success") { 
            ...
            console.error("Ошибка входа: неверные учетные данные или другая ошибка сервера"); 
        } 
    } catch (error) { 
